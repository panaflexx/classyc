 ...
 * replaced by  || decl->decl_spec.thread_local_p,
                     TRUE);
   qsort (VARR_ADDR (init_el_t, init_els) + init_start,
          VARR_LENGTH (init_el_t, init_els) - init_start, sizeof (init_el_t), cmp_init_el);
   if (decl->scope == top_scope || decl->decl_spec the textual value of the embedded C expression; `{{` and `}}`
...
int main() {
    ...
    check(strcmp(s2, "hello bob, age 31") == 0, "2  multiple {} with text");
