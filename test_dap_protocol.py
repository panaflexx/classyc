        ...
        while time.time() < deadline:
            try:
                ...
                if (msg.get("type") == "response" and
                    msg.get("request_seq") == expected_request_seq):
                    return msg
            except queue.Empty:
                continue
        return None
...
def main():
    ...
    try:
        ...
        if not resp:
            ...
            return 1
        print(f"<- initialize response: seq={resp.get('seq')}, request_seq={resp.get('request_seq')}, success={resp.get('success')}")
        ...
        init_event = next((m for m in collected if m.get("type")=="event" and m.get("event")=="initialized"), None)
        if not init_event:
        ...
        assert init_event["seq"] == 2, f"initialized event should have seq=2, got {init_event['seq']}"
        ...
        print(f"<- configurationDone response: seq={resp['seq']}, request_seq={resp['request_seq']}")
        ...
        print(f"<- launch response: seq={resp['seq']}, request_seq={resp['request_seq']}")
        ...
        for m in run_events:
            if m.get("type") == "event":
                ...
                print(f"<- (unexpected) response seq={m.get('seq')} req={m.get('request_seq')}")
        ...
        if resp:
            print(f"<- threads response: seq={resp['seq']}, request_seq={resp['request_seq']}")
            assert resp["request_seq"] == threads_seq

        # === 5. disconnect ===
        disc_seq = client.send_request("disconnect", {
            "restart": False,
        ...
        if resp:
            print(f"<- disconnect response: seq={resp['seq']}, request_seq={resp['request_seq']}, success={resp.get('success')}")
        ...
        print("PASS: server maintained independent, strictly increasing seq counter (1,2,3,...)")
        print(f"PASS: all responses carried correct request_seq values matching the requests we sent.")
        print(f"PASS: client seqs ({list(range(1, client.client_seq))}) were independent of server seqs.")
        ...
        assert any(m.get("event") == "terminated" for m in client.received_messages if m.get("type")=="event")
